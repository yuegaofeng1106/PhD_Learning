...
def nn_train(EV_data, cs_cnt):   #双层可信联邦学习
    ...
    if(use_gpu):  #是否使用GPU训练
        ...
        fl_model = model 
        loss_fn = nn.MSELoss()     
    opt = optim.SGD(params=model.parameters(), lr=0.01) #设定模型参数
    list_number = [0]    #为了重复数据集
    models = []
    iter_loss, loss_sum = np.zeros([100,99]), 0  #为了绘制准确度和损失值
    ...
    for iter in range(100):     #设定迭代次数
        ...
        for cs in range(cs_cnt): #可以控制双层的客户机数量
            if iter == 0:          #这个非常重要  要不然每次都重新建立工人，太费时间了
                ...
                datasets = data_distribution(cs_data)        #数据分发给客户端,返回充电站一个的模型，一步的结果
            ...
            for data, target in datasets:# 遍历每个工作机的数据集
                ...
                loss_sum += loss.get().detach().numpy() #为了画图
                opt.step()            # 更新参数
                model.get()            # 获取充电站模型
                
            print(loss_sum)
            iter_loss[iter,cs+1] = loss_sum
            loss_sum = 0
            models += [model]    #合并每个充电站的模型 组成一个列表
        iter_loss[iter,0] = iter  #做个序列号，出图方便
        fl_model = fl_wgt_avg(models, model)    #得到全部充电站的信息，第二层加权联邦学习进行
        models = []   #第二次就得清除上面的模型了，要不然累积
    np.savetxt('fedlearn/iter_loss.csv', iter_loss, fmt="%s", delimiter=',')      
    #输出总的模型        
    return fl_model
