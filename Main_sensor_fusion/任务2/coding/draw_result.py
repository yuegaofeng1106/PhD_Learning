...
if __name__ == "__main__":
    ...
    # 初始化各种变量
    N = 4            # 传感器数量
    delta_t = 10      # 时间差定义为6，可变化
    lamda = 4        # 分组个数定义为4
    t = 0            # 启动时刻
    seq = len(dataset)
    W = np.array([])  # 权值向量,后期可加
    result = np.zeros([seq, 1])
    while(t < seq):  # 无法启动聚类滤波核时
        if (t < delta_t):
            ...
            plt.xlabel('时间(10ms))',fontsize=18)
            plt.ylabel('Z轴加速度((m/s)/s)',fontsize=18)
            colors=['b','g','r','m','g','r','m']
            markers=['o','s','D','+','s','D','+']
            for i, j in enumerate(km.labels_):
                plt.scatter(t,data[i][j-2], color=colors[j], marker=markers[j], s=6)
        t += 1    # 每次往前走一步
